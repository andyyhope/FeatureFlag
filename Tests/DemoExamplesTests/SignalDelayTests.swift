...
final class SignalDelayTests: XCTestCase {

    func testTheOfferedDelaysAreInstantThreeFiveAndTen() {
        XCTAssertEqual(SignalDelay.allCases.map(\.rawValue), [0, 3, 5, 10])
    }

    func testInstantIsLabelledAsSuchAndTheRestInSeconds() {
        XCTAssertEqual(SignalDelay.allCases.map(\.label), ["Instant", "3s", "5s", "10s"])
    }
    ...
    func testInstantIsTheOnlyZeroDelay() {
        ...
        XCTAssertEqual(SignalDelay.allCases.filter { $0.rawValue == 0 }, [.instant])
    }

    func testDelaysAreDistinctAndAscending() {
        let values = SignalDelay.allCases.map(\.rawValue)
